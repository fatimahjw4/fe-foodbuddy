...
// PAGE AUTH GUARD
// ===============================
// authFetch(`${API_BASE_URL}/api/admin/profile`)
//   .catch(() => {}); 
...
async function openModal(foodId = null) {
    ...
    if (foodId) {
        ...
        try {
            const response = await authFetch(`${API_BASE_URL}/makanan/${foodId}`);
...
async function confirmDelete(id, foodName) {
    ...
    if (result.isConfirmed) {
        try {
            const res = await authFetch(`${API_BASE_URL}/makanan/${id}`, { method: 'DELETE' });
